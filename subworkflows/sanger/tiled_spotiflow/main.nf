...
process Spotiflow_call_peaks {
    ...
    container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
        ...
        "quay.io/bioinfotongli/tiled_spotiflow:${container_version}"}"
    containerOptions = {
        workflow.containerEngine == "singularity" ? "--cleanenv --nv -B ${params.spotiflow_model_dir}:/tmp/models":
        ( workflow.containerEngine == "docker" ? "--gpus all -v ${params.spotiflow_model_dir}:/tmp/models": null )
    }
    ...
    """
    export SPOTIFLOW_CACHE_DIR=/tmp/models
